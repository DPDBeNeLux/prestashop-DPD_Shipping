	...
	this.locator = function(objConfig){
		...
		var renderMarkers = function (places){
			...
			if (places.length == 1) {
				...
				xmlhttp.open('POST', objConfig.ajaxpath, false); // + "parcelshopfinder.php?findParcelShops=" + filter + "&long=" + selectedLocation.lng() + "&lat=" + selectedLocation.lat(), false);
				...
				if(xmlhttp.status == 404 || testResult == "FALSE" || testResult == "") {
					...
					for (var i = 0, shop; shop = objResult.ParcelShops[i]; i++) {
						var shopActive = (typeof objConfig.country == 'undefined') || shop.ShopAddress.Country == '' || shop.ShopAddress.Country == objConfig.country;
						
						DPDShops[shop.ParcelShopID] = {
							name 	: shop.ShopAddress.Company,
							street 	: capitaliseFirstLetter(shop.ShopAddress.Street),
							houseNo	: shop.ShopAddress.HouseNo,
							country	: shop.ShopAddress.Country,
							zipCode	: shop.ShopAddress.ZipCode,
							city	: capitaliseFirstLetter(shop.ShopAddress.City)
						};
						...
						var shopLocation = new google.maps.LatLng(shop.GeoData.Latitude, shop.GeoData.Longitude);
						
						// Create shop information bubble (depending on the information)
						var shopInfo = 	'<div class="shopInfoContainer">';
						shopInfo 	+= 	'	<h1>' + shop.ShopAddress.Company + '</h1>';
						// Uncomment the line below if you want the shop id to be displayed (only for internal use!!)
						//shopInfo 	+= 	'	<h2>' + shop.ParcelShopID + '</h2>';						
						shopInfo 	+= 	'	<p>' + capitaliseFirstLetter(shop.ShopAddress.Street) + ' ' + shop.ShopAddress.HouseNo + ', ' + shop.ShopAddress.ZipCode + ' ' + capitaliseFirstLetter(shop.ShopAddress.City) + '</p>';
						
						shopInfo	+= '	<div class="openingHours">';
						if(typeof shop.OpeningHoursList != 'undefined' && typeof shop.OpeningHoursList.OpeningHours != 'undefined'){
							shopInfo	+= '		<h2>' + t(2) + '</h2>';
							shopInfo	+= '		<table>';
							for(var j = 0, day; day = shop.OpeningHoursList.OpeningHours[j]; j++) {
								shopInfo	+= '			<tr>';
								shopInfo	+= '				<td>' + t(wdt(day.WeekDay)) + '</td>';
								if(!day.OpenTimeList.OpenTimeType.length){
									if(day.OpenTimeList.OpenTimeType.TimeFrom.substr(0,2) < 12){
										shopInfo += '				<td>' + day.OpenTimeList.OpenTimeType.TimeFrom + '</td>';
										if(day.OpenTimeList.OpenTimeType.TimeEnd.substr(0,2) > 12){
											shopInfo += '<td></td><td></td><td>' + day.OpenTimeList.OpenTimeType.TimeEnd + '</td>';
										} else {
											shopInfo += '<td>' + day.OpenTimeList.OpenTimeType.TimeEnd + '</td><td></td><td></td>';
										}
									} else {
										shopInfo += '				<td></td><td></td><td>' + day.OpenTimeList.OpenTimeType.TimeFrom + '</td>';
										shopInfo += '<td>' + day.OpenTimeList.OpenTimeType.TimeEnd + '</td>';
									}
								} else {
									for(var k = 0, openTime; openTime = day.OpenTimeList.OpenTimeType[k]; k++){
										shopInfo += '				<td>' + openTime.TimeFrom + '</td><td>' + openTime.TimeEnd + '</td>';
									}
								shopInfo	+= '			</tr>';
							}
						}
						...
						var marker = new google.maps.Marker({
							map: map,
							icon: DPDimage,
							title: shop.ShopAddress.Company,
